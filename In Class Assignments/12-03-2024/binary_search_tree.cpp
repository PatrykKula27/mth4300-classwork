...
int BinarySearchTree::returnHeight(Node* start)
{
    if (!start)
    {
        return -1;
    ...
    int heightLeft = returnHeight(start->left);
    int heightRight = returnHeight(start->right);

    return std::max(heightLeft, heightRight) + 1;
}

int BinarySearchTree::returnHeight() // Wrapper function for returnHeight()
{
